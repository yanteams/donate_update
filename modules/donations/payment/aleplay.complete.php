...
$config = array(
    ...
    "callbackUrl" => "https://nukevietnam.com/wallet/complete/?payment=aleplay",
...
if (isset($_REQUEST['data']) && isset($_REQUEST['checksum'])) {
    ...
    $utils = new NukeViet\Alepay\Utils\AlepayUtils();
...
$TransactionInfo = json_decode($alepay->getTransactionInfo($obj_data->data));

unset($_GET["data"]);

if (strlen($encryptKey) > 0 and $_GET["checksum"] != "" and $_GET["data"] != "") {
...
if ($hashValidated != "CORRECT") {
    ...
    $amount = null2unknown($TransactionInfo->amount); // So tien thanh toan
    $orderInfo = null2unknown($TransactionInfo->orderCode); // Ma hoa don (ID dat hang)
    $txnResponseCode = null2unknown($TransactionInfo->status); // Ma tra ve
    $aleplay_MerchTxnRef = null2unknown($TransactionInfo->orderCode); // Ma giao dich do OnePage Sinh ra dung de QueryDR sau nay
    $payment_id = $TransactionInfo->transactionCode;

    if ($hashValidated == "CORRECT" and $txnResponseCode == "000") {
        // Giao dich thanh cong
        $nv_transaction_status = 4;
    } elseif ($hashValidated == "CORRECT" and $txnResponseCode == "999") {
        // Hủy giao dịch
        $nv_transaction_status = -1;
    } elseif ($hashValidated == "INVALID HASH" and $txnResponseCode == "000") {
        // Tam giu
    ...
    $responseData['transaction_time'] = $TransactionInfo->transactionTime;

    $responseData['transaction_data'] = array('aleplay_MerchTxnRef' => $aleplay_MerchTxnRef);
