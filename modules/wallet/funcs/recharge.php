    ...
    if ($payment == "sms") {
        ...
        if ($payment == 'epay') {
            ...
            if ($checkss == md5($payment . $global_config['sitekey'] . session_id())) {
                ...
                $post['customer_info'] = "";
                $post['customer_anonymous'] = $nv_Request->get_int('customer_anonymous', 'post', '');
                $post['customer_company'] = $nv_Request->get_title('customer_company', 'post', '');
                ...
                if (!empty($post['customer_email']) and !empty($check_valid_email)) {
                    ...
                    $post['id'] = $db->insert_id("INSERT INTO " . $db_config['prefix'] . "_" . $module_data . "_transaction (
                        ...
                        customer_email, customer_phone, customer_address, customer_info, customer_anonymous,customer_company, transaction_id, transaction_type, transaction_status, transaction_time,
                        ...
                        " . $post['money_revenue'] . ", 1, 0, " . $post['customer_id'] . ", " . $db->quote($post['customer_name']) . ",
                        " . $db->quote($post['customer_email']) . ", " . $db->quote($post['customer_phone']) . ", " . $db->quote($post['customer_address']) . ",
                        " . $db->quote($post['customer_info']) . "," . $db->quote($post['customer_anonymous']) . ", " . $db->quote($post['customer_company']) . ",'', -1, 0, 0, " . $db->quote($post['transaction_info']) . ", " . $db->quote($post['transaction_data']) . ",
                ...
                $post['customer_anonymous'] = 0;
                $post['customer_company'] = '';
                $post['money_amount'] = 0;
                $post['money_other'] = 0;
                $post['money_unit'] = '';
                $post['check_term'] = 0;
