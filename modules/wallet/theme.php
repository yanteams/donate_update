...
function nv_show_list_total($COUNT_CANHAN)
{
    ...
    $xtpl->assign('COUNT_CANHAN',$COUNT_CANHAN);
...
function nv_wallet_history_exchange($array, $generate_page, $page, $per_page)
{
    ...
    foreach ($array as $row) {
        ...
        $customer_anonymous = $row['customer_anonymous'];
        $row['customer_name'] = ($customer_anonymous==1) ? $lang_module['customer_anonymous_1'] : ($row['customer_name']);
        $row['customer_company'] = ($row['customer_company']=='') ? $lang_module['customer_canhan'] : ($row['customer_company']);
