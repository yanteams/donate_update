...
if( !empty( $array_table ) and file_exists( NV_ROOTDIR . '/includes/class/PHPExcel.php' ) )
{
	$Excel_Cell_Begin = 5;
	$objPHPExcel = PHPExcel_IOFactory::load( NV_ROOTDIR . '/modules/nvtools/export.xls' );
	...
	$style_fill = array( 'fill' => array(
			'type' => PHPExcel_Style_Fill::FILL_SOLID,
			'startcolor' => array( 'argb' => 'EEEEEE', )
		) );
	...
	foreach( $array_table as $table => $values )
	{
		...
		foreach( $values as $column => $value )
		{
			++$r;
			++$j;
			$objWorksheet->getRowDimension()->setRowHeight(-1);
		...
		$objWorksheet->getStyle( 'A' . $k . ':D' . ($r) )->getBorders( )->getAllBorders( )->setBorderStyle( PHPExcel_Style_Border::BORDER_THIN );
	}
